...
cluster_exp1<- glm(inmigrante ~ hombre + IPCFR + OCUPADO + CASADO + PROPIETARIO+ HIJO_DUMMY + edad   + medio + alto ,data = cluster1_expulsion,family = "binomial")
...
cluster2_expulsion<- cluster2_expulsion %>% mutate(inmigrante=case_when(cluster_origen==2  &  CH15==3 ~1,
                                                                        cluster_destino==2 & CH15 %in% c(1,2) ~0))
cluster2_expulsion<- cluster2_expulsion %>% filter(!is.na(inmigrante))

cluster_exp2<- glm(inmigrante ~ hombre + IPCFR+ HIJO_DUMMY  + CASADO+ PROPIETARIO + OCUPADO  +edad + edad2  + medio + alto ,data = cluster2_expulsion,family = "binomial")
...
cluster_exp3<- glm(inmigrante ~ hombre + IPCFR + HIJO_DUMMY + CASADO  + OCUPADO + edad + edad2  + medio + alto ,data = cluster3_expulsion,family = "binomial")
