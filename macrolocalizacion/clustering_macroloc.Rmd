...
```{r}
inmigrantes_cluster1<- left_join(ephtotal, cluster_destino)
inmigrantes_cluster1<- left_join(inmigrantes_cluster1,cluster_nombres_eph_ch15)
inmigrantes_cluster1<- inmigrantes_cluster1 %>% filter(CH03==1 & CH15%in% c(1,2,3))
inmigrantes_cluster1<- inmigrantes_cluster1 %>%  mutate(inmigrante_oc1=case_when(cluster_origen==1  &  CH15==3 ~1,
                                                                             TRUE ~0))
inmigrantes_cluster1 %>% group_by(inmigrante_oc1) %>% count()
inmoc1<- glm(inmigrante_oc1 ~ edad   + medio + alto ,data = inmigrantes_cluster1,family = "binomial")
summary(inmoc1)
```
```{r}
inmigrantes_cluster2<- left_join(ephtotal, cluster_destino)
inmigrantes_cluster2<- left_join(inmigrantes_cluster2,cluster_nombres_eph_ch15)
inmigrantes_cluster2<- inmigrantes_cluster2 %>% filter(CH03==1 & CH15%in% c(1,2,3))
inmigrantes_cluster2<- inmigrantes_cluster2 %>%  mutate(inmigrante_oc2=case_when(cluster_origen==2  &  CH15==3 ~1,
                                                                             TRUE ~0))
inmigrantes_cluster2 %>% group_by(inmigrante_oc2) %>% count()
inmoc2<- glm(inmigrante_oc2 ~ hombre + POBRE + OCUPADO_BAJO+OCUPADO_ALTO + CASADO + PROPIETARIO+ HIJO_DUMMY + edad   + medio + alto ,data = inmigrantes_cluster2,family = "binomial")
summary(inmoc2)
```
...
cluster_exp1<- glm(inmigrante ~  hombre + POBRE + SUBSIDIO + OCUPADO_BAJO+OCUPADO_ALTO + CASADO + PROPIETARIO+ HIJO_DUMMY + edad   + medio + alto ,data = cluster1_expulsion,family = "binomial")
...
cluster_exp2<- glm(inmigrante ~ hombre + POBRE + SUBSIDIO + OCUPADO_BAJO+OCUPADO_ALTO + CASADO + PROPIETARIO+ HIJO_DUMMY + edad  +edad2 + medio + alto, data= cluster2_expulsion,family = "binomial")
...
cluster3_expulsion<- cluster3_expulsion %>% mutate(inmigrante=case_when(cluster_origen==3  &  CH15==3 ~1,
                                                                        cluster_destino==3 & CH15 %in% c(1,2) ~0))
cluster3_expulsion<- cluster3_expulsion %>% filter(!is.na(inmigrante))

cluster_exp3<- glm(inmigrante ~ hombre + POBRE + SUBSIDIO +  OCUPADO_BAJO+OCUPADO_ALTO + CASADO + PROPIETARIO+ HIJO_DUMMY + edad   + medio + alto ,data = cluster3_expulsion,family = "binomial")
...
\\[-1.8ex] & (1) & (2) & (3)\\ 
\hline \\[-1.8ex] 
 hombre & $-$0.027 & 0.073$^{***}$ & 0.044$^{**}$ \\ 
  & (0.042) & (0.018) & (0.020) \\ 
  & & & \\ 
 POBRE & 0.148$^{***}$ & $-$0.016 & $-$0.440$^{***}$ \\ 
  & (0.041) & (0.017) & (0.020) \\ 
  & & & \\ 
 OCUPADO\_BAJO & $-$0.143$^{***}$ & $-$0.127$^{***}$ & $-$0.097$^{***}$ \\ 
  & (0.047) & (0.019) & (0.023) \\ 
  & & & \\ 
 OCUPADO\_ALTO & $-$0.226$^{***}$ & $-$0.127$^{***}$ & $-$0.066$^{**}$ \\ 
  & (0.061) & (0.027) & (0.027) \\ 
  & & & \\ 
 CASADO & 0.068 & 0.064$^{***}$ & $-$0.054$^{***}$ \\ 
  & (0.043) & (0.018) & (0.020) \\ 
  & & & \\ 
 PROPIETARIO & $-$0.267$^{***}$ & $-$0.493$^{***}$ & 0.010 \\ 
  & (0.039) & (0.017) & (0.019) \\ 
  & & & \\ 
 HIJO\_DUMMY & $-$0.411$^{***}$ & $-$0.388$^{***}$ & 0.008 \\ 
  & (0.039) & (0.017) & (0.019) \\ 
  & & & \\ 
 edad & 0.013$^{***}$ & 0.010$^{***}$ & 0.007$^{***}$ \\ 
  & (0.001) & (0.001) & (0.001) \\ 
  & & & \\ 
 medio & 0.208$^{***}$ & $-$0.318$^{***}$ & 0.263$^{***}$ \\ 
  & (0.042) & (0.018) & (0.020) \\ 
  & & & \\ 
 alto & 0.540$^{***}$ & $-$0.371$^{***}$ & 0.368$^{***}$ \\ 
  & (0.055) & (0.025) & (0.024) \\ 
  & & & \\ 
 Constant & $-$2.063$^{***}$ & $-$1.229$^{***}$ & $-$2.069$^{***}$ \\ 
  & (0.097) & (0.041) & (0.047) \\ 
  & & & \\ 
\hline \\[-1.8ex] 
Observations & 25,044 & 116,512 & 118,754 \\ 
Log Likelihood & $-$10,640.860 & $-$56,345.440 & $-$49,797.470 \\ 
Akaike Inf. Crit. & 21,303.720 & 112,712.900 & 99,616.950 \\ 
\hline 
\hline \\[-1.8ex] 
