...
Para esta aglomeración se utilizará el algoritmo de K-medias (*K-means*), y el indicador a utiliar para definir el numero de regiones óptimas será el *Within-cluster sum of square (WSS)*.

El WSS es un indicador que mide la suma de las distancias entre las variables dentro de  los cluster y sus centroides, en donde el objetivo es es minimizar dicha discrepancia dentro de cada grupo, teniendo en cuenta el *trade-off* que implica cuando este es mínimo, el cual es el caso en que el numero de clusters es igual al número de variables a clusterizar, en donde si tenemos j observaciones, el numero de clusters sería tal que j=k, y no se estaría dando ninguna información relevante a los efectos de poder resumir caracteristicas comunes entre los grupos, esto provoca que se tenga que tomar una decisión en el numero de regiones óptimas, tal que las provincias dentro de cada cluster sean lo más similares posibles en términos de factores de macrolozalización, pero teniendo en cuenta que una división muy extensa de regiones pierde utilidad analítica a los efectos de definir zonas con caracterísiticas similares para un posterior análisis de movilidad regional.
