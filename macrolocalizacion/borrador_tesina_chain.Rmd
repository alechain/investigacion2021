...
cluster1_diagrama<- cluster1_multinom %>% mutate(flujo_de_1=case_when(cluster_origen==1  &  CH15==3 &   cluster_destino==1 ~ 1,
                                                                      cluster_origen==1  &  CH15==3 &   cluster_destino==2 ~2,
                                                                      cluster_origen==1  &  CH15==3 &   cluster_destino==3 ~3))
  
cluster1_diagrama<- cluster1_diagrama %>% group_by(flujo_de_1) %>% summarise(cantidad=sum(PONDERA))
...
cluster2_diagrama<- cluster2_multinom %>% mutate(flujo_de_2=case_when(cluster_origen==2  &  CH15==3 &   cluster_destino==1 ~ 1,
                                                                      cluster_origen==2  &  CH15==3 &   cluster_destino==2 ~2,
                                                                      cluster_origen==2  &  CH15==3 &   cluster_destino==3 ~3))
  
cluster2_diagrama<- cluster2_diagrama %>% group_by(flujo_de_2) %>% summarise(cantidad=sum(PONDERA))
...
cluster3_diagrama<- cluster3_multinom %>% mutate(flujo_de_3=case_when(cluster_origen==3  &  CH15==3 &   cluster_destino==1 ~ 1,
                                                                      cluster_origen==3  &  CH15==3 &   cluster_destino==2 ~2,
                                                                      cluster_origen==3  &  CH15==3 &   cluster_destino==3 ~3))
  
cluster3_diagrama<- cluster3_diagrama %>% group_by(flujo_de_3) %>% summarise(cantidad=sum(PONDERA))
...
stargazer(cluster_multinom_model)
```
...
frow<- cluster1_diagrama %>% filter(!is.na(flujo_de_1)) 

frow<- as.matrix(frow)
frow<- frow[,2]

srow<- cluster2_diagrama %>% filter(!is.na(flujo_de_2)) 

srow<- as.matrix(srow)
srow<- srow[,2]

trow<- cluster3_diagrama %>% filter(!is.na(flujo_de_3)) 

trow<- as.matrix(trow)
trow<- trow[,2]

m<-matrix(c((frow),(srow), (trow)), ncol = 3)
m<-t(m)
rownames(m)<- paste0("R.O ", 1:3)
colnames(m)<- paste0("R.D ", 1:3)
m
color[3]
colors <- c( "R.D 1" = "#e41a1c", "R.D 2" ="#377eb8",
           "R.D 3" = "#4daf4a", "R.O 1" = "#e41a1c",
             "R.O 2" ="#377eb8",  "R.O 3" = "#4daf4a")


#pdf(file = "/home/alejandrochain/Documentos/GitHub/investigacion2021/latex/graficos/chord.pdf",height = 4, width = 6)
#tikz("/home/alejandrochain/Documentos/GitHub/investigacion2021/latex/graficos/chord.tex",width = 6, height = 4,sanitize = TRUE)
chordDiagram(m, annotationTrack =  c("name", "grid", "ticks"), transparency = 0.2, grid.col = colors)
#dev.off()
```
