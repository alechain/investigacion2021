...
setwd('/Users/achain/Documents/github/investigacion2021/macrolocalizacion')
...
tibble(datos_norm=c(datos_norm[,6])) %>% 
  ggplot(mapping=aes(x=datos_norm)) + geom_histogram(bins = 12) + labs(title = 'Pruebita')
```
...
mean(cantidad_migrantes$`sum(PONDERA[CH16 == 3])`)
  summarise(sum(PONDERA)) %>% pull()/ephtotal%>% filter(CH15  %in% c(1,2,3))  %>% summarise(sum(PONDERA)) %>% pull()
